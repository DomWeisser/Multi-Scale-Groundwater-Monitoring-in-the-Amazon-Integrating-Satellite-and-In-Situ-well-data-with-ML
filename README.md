...
# Project Overview
Despite the Amazon region’s vast surface water resources, over two-thirds of its urban population rely on groundwater for domestic and industrial needs. One of Brazil’s most important freshwater reserves is the Alter do Chão Aquifer (ACA), which stretches beneath parts of the Amazonas, Pará, and Amapá states. However, the monthly patterns and overall behavior of groundwater storage in this aquifer remain poorly understood, especially the link between large-scale satellite-based storage data and local well-level measurements. Recent extreme droughts (notably in 2016 and 2023) have underscored the urgent need for better insight into these dynamics.
This project presents a proof-of-concept machine learning (ML) framework designed to predict month-on-month changes in groundwater levels of wells using satellite observations. The goal is to explore the relationship between GRACE-derived regional water storage estimates (with coarse spatial resolution of 1° x 1°) and localised well data, supported by Sentinel-1 radar backscatter and seasonal trend analysis.
Due to the remoteness of the study area, only four wells provided monthly groundwater data for 2019. I averaged these readings to represent the region as a whole, a necessary simplification given the sparse data coverage. Similarly, average Sentinel-1 radar backscatter was used to estimate surface moisture across the region, which is predominantly rainforest and therefore relatively uniform in land cover.
Despite these data limitations, the model achieved strong performance, explaining 81% of the variance in month-to-month groundwater changes when using cross-validation techniques(R² = 0.81). Predictions include both point estimates and quantified uncertainty using Bayesian and bootstrap methods. While constrained by limited data, the results are promising and suggest the potential for further development with expanded access to well data and longer observation periods.

# Study Area
My analysis focuses on a ~ 280km x 220km area around Manaus, Brazil (60.8°W–58.2°W, 3.7°S–1.7°S), chosen for its hydrogeological diversity and the opportunity it offers to integrate satellite data with limited well-based observations. This region includes the high-porosity sandy formations typical of the Alter do Chão Aquifer and experiences distinct wet (November–April) and dry (May–October) seasons, making it well-suited for studying seasonal groundwater behavior. Outside of Manaus, ground monitoring infrastructure is extremely limited, highlighting the importance of remote sensing and reliable uncertainty quantification to track groundwater trends in data-scarce environments.
...
- In-situ Wells: Groundwater level measurements are obtained from the RIMAS monitoring network, which tracks aquifer behavior across Brazil. For this study, the dataset includes monthly groundwater level changes from four wells within the region of interest. These point-based measurements serve as the target variable for model training, providing a direct, ground-truth reference for assessing and validating satellite-based predictions of groundwater storage.
...
The GRACE dataset provides Total Water Storage (TWS) anomalies from a baseline, which represent deviations from the long-term average of all water stored on and beneath the land surface. This includes groundwater, soil moisture, surface water, snow, and canopy water. To isolate groundwater storage, I use the Global Land Data Assimilation System (GLDAS), which provides modeled estimates of surface water components. These include soil moisture (at multiple depths: 0–10 cm, 10–40 cm, 40–100 cm, and 100–200 cm), snow water equivalent (SWE), and canopy water content. By subtracting these surface components from the GRACE TWS anomalies, I estimate the Groundwater Storage Anomaly (GWSA) using a water balance approach:
...
For my analysis I have averaged the sentinel-1 backscatter (VV and VH polarisations) across the whole region of interest which is a oversimplification the graphs below show how these raw and normalised averages change throughout the year. While the raw backscatter data shows no huge fluctuations, when looking at the normalised graph these small changes align with the seaonal 

Monthly averages of Sentinel-1 backscatter (VV and VH polarisations) are used to capture surface conditions that may relate to underlying groundwater dynamics. The raw backscatter data shows relatively stable values throughout the year with minimal variation, but when normalized, both polarizations reveal distinct seasonal cycles with peaks in January (wet season onset) and July/October (dry season), and minima in March and August. These normalized patterns align well with Amazon seasonality, where the January peak corresponds to increased surface water and vegetation density during wet season onset (December-May), while the July peak captures peak dry season vegetation stress when canopy structural changes are most pronounced. The October VH peak coincides with the dry-to-wet season transition when vegetation begins responding to increasing precipitation. While this represents a significant oversimplification given the vast heterogeneity of the Amazon basin and the complex radar-vegetation interactions in tropical rainforests, the clear seasonal signal suggests that Sentinel-1 can provide meaningful information about surface hydrological changes that may correlate with deeper groundwater processes.
